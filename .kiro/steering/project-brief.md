...
> **Status:** Phase 1 (live web app) COMPLETE — Node/Express `crime_api` + React Dashboard run locally on the bundled CSV fallback; Data Store loader + Catalyst deploy config ready (user runs `catalyst deploy`). Next: Phase 2 (Hotspot Map + spatiotemporal + modeled time-of-day layer).
...
- [ ] **Phase 2 (next session):** deep Hotspot Map (KDE + DBSCAN on `agg_hotspots`) + station drill-down + modeled time-of-day layer (labelled).
